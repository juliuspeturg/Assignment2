...
```
So now we ca use ower function to analize the heat consumption time series.
```{r chunk2}
acf.plot(VEKS$HC.c,0,168)
```
...
As we can see in `r fig$ref("plot1",link=F)` the ACF tapers of very slowly, this indicats tha the timeseries is nonstationari and we shoud diffirenciat to be able to extract any usueful information from tha plots so next we differenciate once and take anther look at the outputs.


```{r chunk3}
acf.plot(VEKS$HC.c,1,168)
```
...
In `r fig$ref("plot2",link=F)` We can se a pattern emerge, we see some intresting spikes in lag 12 and lag 24 lag 36 and so on.Both the ACF and PACF show sem cyclical behavior. If we look at the spectrum plotof the series we se two destinct spices with frequency of around 0.4 and 0.8. the inverse of thees frequencyc if the periog of 12hours and 24hours.
